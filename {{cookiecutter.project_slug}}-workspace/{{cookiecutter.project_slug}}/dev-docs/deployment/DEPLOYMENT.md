...
This document outlines the deployment strategy for distributing {{ cookiecutter.command_name }} to users.
...
2. Add new trusted publisher:
   ...
   - Repository: `{{ cookiecutter.command_name }}`
...
# Test installation
pip install --index-url https://test.pypi.org/simple/ --no-deps {{ cookiecutter.command_name }}
...
Create separate tap repository: `homebrew-{{ cookiecutter.command_name }}`

**Repository**: `https://github.com/{{ cookiecutter.github_username }}/homebrew-{{ cookiecutter.command_name }}`

### Formula Template

File: `Formula/{{ cookiecutter.command_name }}.rb`
...
class Uniqseq < Formula
  ...
  test do
    ...
    output = shell_output("#{bin}/{{ cookiecutter.command_name }} --window-size 3 --quiet #{testpath}/test.txt")
    assert_equal "line1\nline2\nline3\n", output
  end
end
```
...
```bash
brew tap {{ cookiecutter.github_username }}/{{ cookiecutter.command_name }}
...
2. Test locally: `brew install --build-from-source ./Formula/{{ cookiecutter.project_slug }}.rb`
3. Commit and push formula update
4. Users update: `brew upgrade {{ cookiecutter.command_name }}`
...
2. **Homebrew Tap** (Current Priority)
   - [ ] Create `homebrew-{{ cookiecutter.command_name }}` repository
