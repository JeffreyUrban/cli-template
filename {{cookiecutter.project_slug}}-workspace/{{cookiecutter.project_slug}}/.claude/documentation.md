...
=== "CLI"
    ...
    ```console
    $ {{ cookiecutter.command_name }} --option value input.txt
    Result: Success
    ```
...
Implementation: `src/{{ cookiecutter.project_slug }}/storage.py`
```
...
❌ **Bad:**
```markdown
{{ cookiecutter.project_slug }} is 10x faster than alternatives.
```

✅ **Good:**
```markdown
{{ cookiecutter.project_slug }} processes 1M records in 2.5s (measured on test dataset).
...
```markdown
The validation logic is in `src/{{ cookiecutter.project_slug }}/validator.py:45`
```
