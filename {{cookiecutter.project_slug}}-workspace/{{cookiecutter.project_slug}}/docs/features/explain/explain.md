...
{{ cookiecutter.command_name }} large.log --explain --progress 2>&1 | tee diagnostics.txt
```
...
{{ cookiecutter.command_name }} log.txt --explain > clean.log 2> explain.log

# Merge for analysis
{{ cookiecutter.command_name }} log.txt --explain 2>&1 | grep "Line 42"
```
